/**
 * Animagine XL 4.0 像素风角色 sprite 出图
 * 用法：node tools/comfy_animagine_pixel.mjs [key1,key2,...]
 * 读 workflow/animagineXL40_portrait.json 骨架，注入像素提示词 + 方形尺寸，下载到 tmp/animagine_pixel/
 */
import { readFileSync, writeFileSync, mkdirSync } from 'fs';

const COMFY_URL = 'http://127.0.0.1:8188';
const WF = JSON.parse(readFileSync('workflow/animagineXL40_portrait.json', 'utf-8'));
const OUT_DIR = 'tmp/animagine_pixel';
...
const PIXELS = {
  xiya_pixel: {
    positive:
      'masterpiece, best quality, very aesthetic, absurdres, pixel art, 16-bit, retro game, game character sprite, 1girl, solo, full body, standing, short orange bob cut hair, warm fair skin, kind steady eyes, orange work top, blue denim apron, leather tool belt, small wrench, old canvas tool bag over shoulder, gentle confident smile, stardew valley inspired, hard pixel edges, limited color palette, clean readable silhouette, subtle shading, no gradients, pure black background, centered, front view, full body head to feet',
    width: 1024, height: 1024,
  },
  linche_pixel: {
    positive:
      'masterpiece, best quality, very aesthetic, absurdres, pixel art, 16-bit, retro game, game character sprite, 1boy, solo, full body, standing, 27 years old, short dark brown hair, calm gentle expression, warm fair skin, casual shirt, light outdoor jacket, jeans, small backpack, former programmer turned farmer, stardew valley inspired, hard pixel edges, limited color palette, clean readable silhouette, subtle shading, no gradients, pure black background, centered, front view, full body head to feet',
    width: 1024, height: 1024,
  },
};
...
(async () => {
  ...
  if (keys.length === 0) keys = ['xiya_pixel', 'linche_pixel'];
