...
export type SfxName =
  ...
  // 第一章 P2 钓鱼 Phase 1（2026-08-14，制作人 Decision Override 启动）：纯手感原型 4 个音效
  // 设计依据：《钓鱼MVP-设计与施工规范-v0.1》§十三 声音设计
  | 'fish_cast' | 'fish_fake_bite' | 'fish_real_bite' | 'fish_success'
  // 第一章 P2 生活采集 Phase 1（2026-08-14 设计稿 v0.1）：采下/折断的轻反馈，零资产
  | 'gather';
...
export function play(name: SfxName): void {
  ...
  switch (name) {
    ...
    case 'crowd':
      // 第一章 P3 春日集人群声（2026-08-12）：柔和低通噪声打底（人群嗡嗡）
      // + 几段不规律短噪声（说话起伏感）+ 极轻三角波（远处嗓音模糊感）。
      // 低音量一次性 3s——"声音告诉玩家：镇上重新聚起了人"。
      noise(3.0, 0.05);
      noise(0.4, 0.03, 0.6);
      noise(0.5, 0.035, 1.4);
    ...
    // ──────────── 钓鱼 Phase 1 程序合成音效（2026-08-14，依据《施工规范》§十三） ────────────

    case 'fish_cast':
      // 甩竿：竿身短促"嗖"声（高频噪声短衰减）+ 鱼线落水"咚"一声（低频闷响 + 极轻水花）
      // 设计：操作反馈明确但不刺耳——"我做了甩竿动作"
      noise(0.08, 0.06);
      tone(180, 0.1, 'sine', 0.1, 0.12);   // 落水低频
      noise(0.06, 0.04, 0.16);              // 落水水花（极轻）
      break;

    case 'fish_fake_bite':
      // 试探假动作：极轻的水面声响（短促低通噪声 + 一点点高频"叮"——浮漂轻碰水）
      // 设计：音量明显低于真咬钩，让玩家逐渐"听声音判断"——"刚动一下不一定是真的"
      noise(0.08, 0.025);
      tone(1200, 0.04, 'sine', 0.018, 0.02); // 浮漂轻碰水
      break;

    case 'fish_real_bite':
      // 真咬钩：明显强于试探——水声 + 短促咬钩音（中频三角形波"咕咚"感）
      // 设计：玩家听到能立即分辨"这是真的"——明显强于 fake_bite
      noise(0.15, 0.07);
      tone(420, 0.1, 'triangle', 0.08);   // 咬钩音
      tone(280, 0.12, 'sine', 0.06, 0.05); // 下沉感
      break;

    case 'fish_success':
      // 成功收竿：整套玩法最重要的声音反馈——收竿 + 水花 + 鱼出现
      // 设计：0.5~0.8s 内多层次反馈（竿拉起 / 鱼线绷紧 / 水花 / 鱼跳出），"我钓到了"的爽感
      tone(660, 0.06, 'triangle', 0.1);    // 收竿拉起感
      noise(0.18, 0.1, 0.06);              // 水花（明显）
      tone(880, 0.08, 'sine', 0.09, 0.12); // 鱼线绷紧高频
      tone(1320, 0.12, 'sine', 0.07, 0.22); // 鱼跳出的"啵"声
      tone(1568, 0.2, 'sine', 0.05, 0.32);  // 成功余韵（高八度光感）
      break;

    case 'gather':
      // 生活采集：极轻的"采下"反馈（设计稿 §六 0.3~0.8s 完整反馈目标）
      // 轻拨/折断的瞬时音 + 一点点空气感，不与 harvest 丰收音冲突
      tone(520, 0.04, 'sine', 0.05);    // 轻拨主音
      noise(0.04, 0.06, 0.04);          // 叶片/枝条轻响
      tone(780, 0.06, 'sine', 0.03, 0.05); // 余韵高频（"采下了"的清晰感）
      break;
