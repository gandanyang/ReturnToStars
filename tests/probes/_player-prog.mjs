...
const save = {
  version: '0.5', savedAt: 'player-check', timestamp: Date.now(),
...
await page.evaluate(() => {
  ...
  s.player.x = 300; s.player.y = 200;
  s.cameras.main.startFollow(s.player, true, 0.5, 0.5, 0, 0);
});
for (const facing of ['down', 'left', 'right', 'up']) {
  await page.evaluate((f) => { const s = window.__game.scene.getScene('farm'); s.player.facing = f; s.player.anims.stop(); s.player.setFrame(f === 'down' ? 0 : f === 'left' ? 4 : f === 'right' ? 8 : 12); }, facing);
  await sleep(500);
  await page.screenshot({ path: `G:/ReturnToStars/tests/probes/test-screenshots/player-prog-${facing}.png` });
}
